...
class TestLoadConfig:
    def test_loads_valid_config(self, config_file):
        ...
        assert config.scraper.page_size == 25
        assert config.scraper.max_pages == 3
        assert len(config.categories) == 1
        assert config.categories[0].name == "IT"
        assert config.categories[0].keywords == "software engineer"
        assert config.categories[0].location == "Singapore"
    ...
    def test_missing_keywords_raises(self, config_file):
        path = config_file(
            'categories:\n  - name: "IT"\n    keywords: ""\n    ntfy_topic: "t"\n'
        )
    ...
    def test_defaults_applied(self, config_file):
        ...
        assert config.scraper.page_size == 25
        assert config.scraper.delay_between_requests == 3.0
    ...
    def test_ai_enabled_without_key_raises(self, config_file):
        path = config_file(
            VALID_CONFIG + "\nai:\n  enabled: true\n  api_key: \"\"\n"
        )
        with pytest.raises(ValueError, match="AI is enabled"):
            load_config(path)

    def test_ai_env_overrides(self, config_file, monkeypatch):
